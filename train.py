...
def get_regular_case(word):
    # returns normal form of any word.
    return morph.parse(word)[0].normal_form


def get_word_order(sentence_list):
    l.info('getting word order')
    # returns:
    #
    ...
    for word_list in sentence_list:
        ...
        for word_id in range(len(word_list) - 1):
            ...
            try:
                ...
                continue
            try:
                order[word]['_static']['pos'] = (order[word]['position'] + word_pos) / 2
                ...
                try:
                    ...
                    try:
                        ...
                        order[word]['_static'] = {'pos': word_pos}
    ...
    order_static = [[i, order[i]['_static']] for i in list(order) if order[i].get('_static') is not None]
    tmporder = order.copy()
    for i in tmporder:
        try:
    ...
    for i in order_static:
        if i[0] in list(order):
            order[i[0]].update({'_static': i[1]})
